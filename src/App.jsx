...
const filtered=(items||[]).filter(x=>x.poster_path).slice(0,12);
if(!filtered.length)return null;
return <div className="space-y-3">
<div className="flex items-center justify-between"><SectionTitle title={title} icon={icon}/><span className="text-[9px] text-[#64748B]">{lang==='ar-SA'?'اسحب للمزيد':'Swipe for more'}</span></div>
<div className="flex gap-3 overflow-x-auto scrollbar-none pb-2">
{filtered.map(item=><div key={`${type}-${item.id}`} className="w-[125px] flex-shrink-0"><MovieCard item={{...item,media_type:item.media_type||type}} onClick={()=>onItemClick(item,item.media_type||type)}/></div>)}
</div>
</div>
};

const HorizontalSection=({title,icon,items,loading,onItemClick,onViewAll,lang})=><div className="space-y-3">
<div className="flex items-center justify-between"><SectionTitle title={title} icon={icon}/>{onViewAll&&<button onClick={onViewAll} className="text-[10px] text-[#60A5FA] font-bold">{lang==='ar-SA'?'عرض الكل ›':'See All ›'}</button>}</div>
{loading?<div className="flex gap-3 overflow-x-auto scrollbar-none">{Array.from({length:4}).map((_,i)=><div key={i} className="w-[120px] h-[180px] bg-[#111827] rounded-2xl animate-pulse flex-shrink-0"/>)}</div>:
<div className="flex gap-3 overflow-x-auto scrollbar-none pb-1">{items.slice(0,12).map(item=><div key={item.id} className="w-[120px] flex-shrink-0"><MovieCard item={item} onClick={()=>onItemClick(item)}/></div>)}</div>}
</div>;
